                ...
                <div class="col-xxl-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            ...
                            <div class="">
                                <a href="{{ route('slider.archive') }}" class="btn btn-sm btn-info">Archive List</a>
                        ...
                        @if ($data->count() == 0)
                            <div class="card-body">
                                <div class="alert alert-info text-center">
                                    <h3>Oops!</h3> No sliders have been created yet.
                                    <a class="hover-underline" href="{{ route('slider.create') }}">Create now ! </a>
                                </div>
                            </div>
                        @endif
...
@section('js')
    <script>
        $(function() {
            $(".sortable").sortable({
                ...
                stop: function() {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('slider.orderChangeHandler') }}",
                        data: postData,
        ...
        function statusChangeHandler(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('slider.statusChangeHandler') }}",
                data: data,
                success: function(response) {
                    if (response.status == 201) {
                        Toastify({
