                ...
                <div class="col-xxl-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            <h4 class="card-title">Archived Page List</h4>
                            <div class="">
                                <a href="{{ route('page.index') }}" class="btn btn-sm btn-info">Turn Back</a>
                            </div>
                        </div>
                        @if ($data->count() == 0)
                            <div class="card-body">
                                <div class="alert alert-info text-center">
                                    <h3>Oops!</h3> No pages have been archived yet.
                                    <a class="hover-underline" href="{{ route('page.index') }}">Turn back ! </a>
                                </div>
                            </div>
                        @endif
                        @if ($data->count() > 0)
...
@section('js')
    <script>
        $(function() {
            $(".sortable").sortable({
                ...
                stop: function() {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('page.orderArchiveChangeHandler') }}",
                        data: postData,
        ...
        function statusChangeHandler(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('page.statusArchiveChangeHandler') }}",
                data: data,
                success: function(response) {
                    if (response.status == 201) {
                        Toastify({
